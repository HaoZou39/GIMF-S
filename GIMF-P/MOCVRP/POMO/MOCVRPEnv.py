...
class CVRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1, problems=None):
        ...
        CHANEl_IDX = torch.arange(self.channels)[None, :, None, None].expand(self.batch_size, self.channels, self.problem_size + 1,
                                                                 self.offsets.shape[0]).contiguous().view(-1)
        BATCH_IDX = torch.arange(self.batch_size)[:, None, None, None].expand(self.batch_size, self.channels, self.problem_size + 1,
