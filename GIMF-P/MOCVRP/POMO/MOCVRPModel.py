...
class CVRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        self.encoded_nodes = self.encoder(depot_xy, node_xy_demand, img)
...
class CVRP_Encoder(nn.Module):
    ...
    def forward(self, depot_xy, node_xy_demand, img):
...
class CVRP_Decoder(nn.Module):
    ...
    def assign(self, pref):
        ...
        self.Wq_last_para = self.hyper_Wq_last(mid_embd[:1 * self.embd_dim]).reshape(head_num * qkv_dim,
                                                                                     embedding_dim + 1)
