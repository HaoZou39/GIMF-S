...
from MOTSProblemDef_3obj import get_random_problems, augment_xy_data_by_n_fold_3obj, augment_preference
...
class TSPEnv:
    ...
    def load_problems(self, batch_size, problem_size, aug_factor=1, problems=None):
        ...
        if aug_factor > 1:
            if aug_factor <= 512:
                ...
                self.instances = augment_xy_data_by_n_fold_3obj(self.instances, aug_factor)
                self.preference = augment_preference(self.preference, aug_factor)
            else:
                raise NotImplementedError

        self.xy_img = torch.ones((self.batch_size, self.channels, self.img_size, self.img_size))
        for i in range(self.channels):
            xy_img = self.instances[:, :, 2 * i: 2 * i + 2] * self.img_size
            xy_img = xy_img.int()
    ...
    def reset(self):
        ...
        return Reset_State(self.instances, self.preference, self.xy_img), reward, done
